...
use crate::{cmds_custom::CustomCMDs, file_json};

pub struct FileHandler {
    path_rt_dir: PathBuf,
    path_cmds_file: PathBuf,
...
impl FileHandler {
    pub fn make_custom_commands(&self, args: &Vec<String>) -> CustomCMDs {
        if !&self.path_rt_dir.exists() {
            fs::create_dir(&self.path_rt_dir).unwrap();
        }
        if !&self.path_cmds_file.exists() {
            let mut file = File::create(&self.path_cmds_file).unwrap();
            file.write_all(&self.default_config_string.as_bytes()).unwrap();
        }
        let custom_commands = fs::read_to_string(&self.path_cmds_file).unwrap();
    ...
    pub fn safe(&self, custom_commands: &CustomCMDs) {
    ...
    pub fn export(&self, args: &Vec<String>) {
        ...
        self.update_file(&self.path_cmds_file, &path_backup_file);
    }

    pub fn import(&self, args: &Vec<String>) {
        let path_backup_file = PathBuf::from_str(&args[2]).unwrap();
        self.update_file(&path_backup_file, &self.path_cmds_file);
    }
